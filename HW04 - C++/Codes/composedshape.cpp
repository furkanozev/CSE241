...
namespace mycomposedshape
{
	...
	void ComposedShape::OptimalFit(Rectangle shape1, Rectangle shape2)
	{
		...
		while(height - y >= height2)
		{
			...
			while(width - x >= width2)
			{
				...
				poly_shapes.push_back(small_shape);
	...
	vector<Polygon> ComposedShape::get_vector()
	{
		return poly_shapes;
	}
	...
	/* Returns the vector's object in the given index.  */
	// Left side
	Polygon& ComposedShape::operator [](unsigned int index)
	{
		// error check
    	if(poly_shapes.size() > index)
    	{
    		return poly_shapes[index];
		}
    	else
    	{
    		exit(1);
	...
	// Right side
	const Polygon& ComposedShape::operator [](unsigned int index) const
	{
		if(poly_shapes.size() > index)
    	{
    		return poly_shapes[index];
	...
	/* for adding a new shape(Point2D) into Polygon vector to this composedShape */
	Polygon ComposedShape::operator +=(Polygon::Point2D &shape)
	{
 		poly_shapes.push_back(shape);
  		return shape;
 	}
	...
	{  
		...
		/* Print inner shapes with using Polygon overloaded function for vector. */
		unsigned int i;
		for(i=0 ; i<shape.poly_shapes.size() ; i++)
		{
			// Call Polygon's << overloaded operator.
			shape.file<<shape[i]; // '[]' index operator used in here instead of shape.poly_shapes[i] .
		}
		shape.getfile()<<"</svg>";
	    return os;
	}  
